...
def after_connect(adb):
    ...
    if(not out):
        ...
        exit()
    whatsapp_apk_path = re.search('(?<=package:)(.*)(?=apk)', str(check_output(
        adb + ' shell pm path com.whatsapp'))).group(1) + 'apk'
    sd_path = getoutput(adb + ' shell "echo $EXTERNAL_STORAGE"')
...
def download_apk(url, file_name):
    ...
    process_bar = tqdm(total=total_size_in_bytes, unit='iB', unit_scale=True)
    ...
    if total_size_in_bytes != 0 and process_bar.n != total_size_in_bytes:
